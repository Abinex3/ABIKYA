...
const [lookupsError, setLookupsError] =
  ...
  comboCatalog,
  setComboCatalog,
] = useState<ComboProduct[]>([]);

const [
  selectedComboProducts,
  setSelectedComboProducts,
] = useState<Record<string, ComboProduct>>(
  {}
);

const [
  comboCatalogLoading,
  setComboCatalogLoading,
] = useState(false);

const [
  comboCatalogError,
  setComboCatalogError,
] = useState("");


const [
  debouncedComboSearch,
  setDebouncedComboSearch,
] = useState("");
...
const [
  existingWornImageUrl,
  setExistingWornImageUrl,
] = useState<string | null>(null);
...
const filteredComboProducts = comboCatalog;
...
const response = await fetch(
  `http://localhost:5000/api/admin/products/combo-options${
    query ? `?${query}` : ""
  }`,
  {
    method: "GET",
...
if (
  ...
  const loadProduct = async () => {
    try {
      ...
      const loadedComboProducts:
  Record<string, ComboProduct> = {};
...
setSelectedComboProducts(
  loadedComboProducts
      ...
      setExistingProductImageUrl(
  product.images?.product?.url ?? null
);

setExistingWornImageUrl(
  product.images?.worn?.url ?? null
);
...
const createProduct = async (
  ...
  if (
    ...
    const fieldErrors =
      data.errors.fieldErrors as Record<
        string,
        string[]
      >;
...
const updateProduct = async () => {
  ...
  const payload = {
    ...
    /*
     * Backend validator currently
     * requires SKU, but the PATCH
     * controller does not update it.
     */
    sku: formData.sku.trim(),
    ...
    /*
     * Unlike Create, Edit keeps
     * the status selected in the form.
     */
    status:
      formData.status,
  ...
  try {
    ...
    /*
     * Upload only newly selected
     * replacement images.
     *
     * If no new File was selected,
     * existing backend images remain.
     */
    if (formData.productImage) {
      setSubmitStep(
...
const deleteExistingProductImage = async (
  type: "PRODUCT" | "WORN"
) => {
  if (!productId) {
    return;
  ...
  const confirmed =
    window.confirm(
      type === "PRODUCT"
        ...
        : "Delete the existing worn image?"
    );
...
if (productLoading) {
  return (
    <div className={`w-full pb-12 ${bodyFont}`}>
      <div className="rounded-[18px] border border-[#e8e3ef] bg-white p-6 text-[13px] text-[#716879]">
        Loading product...
      </div>
    </div>
  );
...
if (productLoadError) {
  return (
    <div className={`w-full pb-12 ${bodyFont}`}>
      <div className="rounded-[18px] border border-[#f4cdd3] bg-[#fff4f5] p-6 text-[13px] text-[#d95c70]">
        {productLoadError}
      </div>
    </div>
  );
...
/>
  ...
  if (
    ...
    void deleteExistingProductImage(
      "PRODUCT"
    );
    return;
  }
